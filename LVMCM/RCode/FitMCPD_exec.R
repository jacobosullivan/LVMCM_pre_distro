...
## This script will estimate the mixing rate parameter from a site by species table
...
## Rscript FitMCPD_exec.R "/data/SBCS-RossbergLab/jack/post_doc/SimulationData/N=20/removal_experiment/2020-8-14/2020-8-14_removal(10000)1.1bMat0.mat" 0 1 1e-4 "ml"

## Example R session implementation:
# args <- c("/data/SBCS-RossbergLab/jack/post_doc/SimulationData/N=20/removalQ_experiment/2020-10-16/2020-10-16_removalQ(10000)1.1bMat0.mat",
#           0, 1, "A", "ml")
...
## Load matrix
comm <- as.matrix(fread(args[1]))
params <- NULL
output <- NULL
if (length(grep("SimulationData", args[1]))>0) { # Test for LVMCM output and load params
  params <- read.table(gsub("bMat", "params", args[1]), as.is=1)
  outexp <- paste0("fitMCPD",args[5],"_",args[4])
  output <- gsub("bMat", outexp, args[1])
  output <- gsub("\\.mat", "\\.csv", output)
} else {
  outexp <- paste0("fitMCPD",args[5]) # assume if not LVMCM, already binary
  output <- gsub("\\.csv", "_fitMCPD\\.csv", args[1])
}

if (as.numeric(args[2])) { ## Transpose matrix
  comm <- t(comm)
}

if (as.numeric(args[3])) { ## Threshold matrix
  comm <- makeBinary(comm, thresh=args[4], params)
}

dat <- f(comm, args[5]) # MCPD fitting procedure
...
if (ASSEMBLEDATA) {
  bFile <- read.table("/data/SBCS-RossbergLab/jack/post_doc/removal/bFileRemoval10-31.txt")
  mFile <- gsub("bMat", "fitMCPDml_D", bFile[,1])
  mFile <- gsub("\\.mat", "\\.csv", mFile)
  ...
  for (i in 1:length(mFile)) {
    dat_rep <- read.csv(mFile[i])
    P <- read.table(gsub("bMat", "params", bFile[i,]), as.is=2)
    sk <- c(sk, as.numeric(P[which(P[,1]=="sk"),2]))
    dispL <- c(dispL, as.numeric(P[which(P[,1]=="dispL"),2]))
    mix <- c(mix, dat_rep$mix[1])
    p_fit <- c(p_fit, dat_rep$p_fit[1])
    obs <- rbind(obs, dat_rep$obs)
    pred <- rbind(pred, dat_rep$pred)
  }
  
  dat <- data.frame(bFile=bFile,
                    sk=sk,
                    dispL=dispL,
                    mix=mix, 
                    p_fit=p_fit)
  dat <- cbind(dat, obs, pred)
  names(dat)[1] <- "bFile"
  names(dat)[6:45] <- paste0(rep(c("obs", "pred"), each=20), 1:20)
  
  write.csv(dat, "/data/SBCS-RossbergLab/jack/post_doc/removal/removal_experiment_fitMCPD_ml_A_10-31.csv", row.names=F)
}
...
if(CACERES) {
  ...
  caceres_dat <- read.csv("~/pCloudDrive/PostDoc/Model_fitting/lichen_fungi/caceres2017-supp_mat.csv")
  
  comm <- caceres_dat[,2:24] # pres-abs species by site matrix
  dat <- f(comm) # MCPD fitting procedure
  write.csv(dat, "/home/jack/pCloudDrive/PostDoc/RemovalExperiment/data/final/caceres_fitMCPD_ml.csv", row.names=F)
}
